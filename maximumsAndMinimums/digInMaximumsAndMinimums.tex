...
\begin{definition}\hfil\index{maximum/minimum!local}
	\begin{enumerate}
		\item A function $f$ has a \dfn{local maximum} at an interior point in its domain $x=a$, if $f(a)\ge
		  f(x)$ for every $x$ near $a$.
		\item A function $f$ has a \dfn{local minimum} at an interior point in its domain $x=a$, if $f(a)\le
		  f(x)$ for every $x$ near $a$.
	\end{enumerate}
	A \dfn{local extremum}\index{extremum!local} is either a local
	maximum or a local minimum.
\end{definition}
In this definition, the phrase "on an interior point of its domain" means that
endpoints are never local extrema.
...
\begin{definition}\index{critical point}
  Suppose $x=a$ is an interior point in the domain of a function $f$.  Then $f$ has a \dfn{critical point} 
  at $x=a$ if 
  \[
  f'(a) = 0\qquad\text{or}\qquad \text{$f'(a)$ does not exist.}
  \]
\end{definition}
Again, the phrase `an interior point in the domain' used here means that the endpoints of the domain are never
critical points.
...
accurate picture. It is worth recalling what we already know.
