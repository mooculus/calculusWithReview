...
Suppose that $\lim_{x\to a}f(x)=L$, $\lim_{x\to a}g(x)=M$, $n$ and $m$ positive integers, and $k$ is a constant.
\begin{description}	
	\item[Sum/Difference Law] $\lim_{x\to a} (f(x) \pm g(x)) =
		  \lim_{x\to a}f(x) \pm \lim_{x\to a}g(x)=L \pm M$.
	\item[Constant Multiple Law] $\lim_{x\to a} kf(x) = k\lim_{x\to a}f(x)=kL$.
	...
	\item[Power Law]  $\lim_{x\to a} (f(x))^n = \left(\lim_{x\to a}f(x)\right)^n=L^n$.
  	\item[Fractional Power Law] $\lim_{x\to a} (f(x))^{n/m} = \left(\lim_{x\to a}f(x)\right)^{n/m}=L^{n/m}$, if $f$ is nonnegative near $a$ when $m$ is even and $n/m$ is reduced.
\end{description}
...
Back in Theorem \ref{theorem:continuity} we mentioned a big list of functions that were continuous.  We mention them again 
in the following statement.  We will study some of these functions in more detail in later sections.  For now, we focus only on
the fact that they are continuous.
\begin{theorem}[Continuity of Other Functions]
	Polynomials, the trigonometric functions $\sin x$ and $\cos x$, and exponential functions $b^x$
	are continuous everywhere.  Rational functions, logarithms, and the other trigonometric functions are continuous in
	their domain.
\end{theorem}

\begin{example}
	Compute the limit:
	\[ \lim_{x\to \pi/4} \sin x \cos x. \]
	
	\begin{explanation}
		By the limit laws, \[ \lim_{x \to \pi/4} \sin x \cos x = \left(\lim_{x\to \pi/4} \sin x \right) \left( \lim_{x\to \pi/4} \cos x \right).\]
		$\displaystyle \lim_{x\to \pi/4} \sin x = \sin \frac{\pi}{4} = \answer[given]{\frac{1}{\sqrt{2}} }$.
		$\displaystyle \lim_{x\to \pi/4} \cos x = \cos \frac{\pi}{4} = \answer[given]{\frac{1}{\sqrt{2}} }$.
		
		Putting these together, $\displaystyle \lim_{x\to \pi/4} \sin x \cos x = \answer[given]{\frac{1}{2}}$.
	\end{explanation}
\end{example}
...
\begin{question}
  ...
  \begin{question}
    Compute:
    \[
    \lim_{x\to 0}\sec^2(5^x-1)\begin{prompt} =\answer{1}\end{prompt}
    \]
    \begin{feedback}
      Notice that
      \[
      \lim_{x\to 0} \sec^2(5^x-1) = \lim_{x\to 0} \frac{1}{\cos^2(5^x-1)}.
      \]
      If we are trying to use Limit Laws to compute this limit, we
      would now have to use the Quotient Law to say that
      \[
      \lim_{x\to 0} \frac{1}{\cos^2(5^x-1)} = \frac{ \lim_{x\to 0}1}{
        \lim_{x\to 0}\cos^2(5^x-1)}.
      \]
      We are only allowed to use this law if both limits exist and the
      ...
      \begin{align*}
        \lim_{x\to 0}\cos^2(5^x-1)  &= \cos^2(\lim_{x\to 0}(5^x-1))\\
        &=\cos^2(\lim_{x\to 0}(5^x)-\lim_{x\to 0}(1))\\
        &=\cos^2(1-1)\\
        &= \cos^2(0)\\
        &=1
      \end{align*}
      ...
      Hence
      \[
      \frac{ \lim_{x\to 0}1}{ \lim_{x\to 0}\cos^2(5^x-1)} =
      \frac{1}{1}=1
      \]
    \end{feedback}
  \end{question}
...
\begin{question}
  Can this limit be directly computed by limit laws?
  \[
  \lim_{x\to 1}{(x-1)\cdot \csc(\sqrt{x^2-1})}
  \]
  \begin{multipleChoice}
    \choice{yes}
    \choice[correct]{no}
  ...
  \begin{feedback}
    ...
    \[
    \lim_{x\to 1}{(x-1)\cdot \csc(\sqrt{x^2-1})}= \lim_{x\to 1}{(x-1)\cdot \lim_{x\to 1}\csc(\sqrt{x^2-1})}.
    \]
...
\begin{question}
  ...
  \[
  \lim_{x\to 0} x\cot^2 x =\lim_{x\to 0} x \cdot \lim_{x\to 0}\cot^2 x.
  \]
  We are only allowed to use this law if both limits exist.  We know
  $\lim_{x\to 0} x = 0$, but what about $\lim_{x\to 0}\cot^2 x$?  We do
  not know how to find $\lim_{x\to 0}\cot^2 x$ using limit laws because $0$
  is not in the domain of $\cot^2 x$.
  \end{feedback}
\end{question}
