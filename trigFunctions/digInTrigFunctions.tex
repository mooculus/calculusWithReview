	...
	\begin{explanation}
		...
		\begin{image}[2in]
		   \begin{tikzpicture}
  		  ...
  		  \end{tikzpicture}
		\end{image}
		From the picture we see $\theta_R = \frac{\pi}{6}$.  In one period $[0, 2\pi]$, there are two angles that have reference angle $\frac{\pi}{6}$ and have negative
...
\begin{example}
	...
	\begin{explanation}
		...
		For $\cos \theta = \frac{1}{2}$, we see that the reference angle $\theta_R = \answer[given]{\frac{\pi}{6}}$.  Since cosine is positive in quadrants 1 and 4,
		we find solutions $\theta = \frac{\pi}{6}$ and $\frac{11\pi}{6}$.
		
		All solutions are:
		\[ \theta = \pi + 2 \pi k, \,\, \frac{\pi}{6}+ 2 \pi k ,\,\, \frac{11\pi}{6} + 2\pi k, \,\, k \textrm{ any integer.} \]
	\end{explanation}
\end{example}
